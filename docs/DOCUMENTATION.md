...
- **No agent dependency for cloud scans**: Paste your AWS/GCP config JSON into the dashboard to get immediate findings without deploying anything.
- **True live data when agent runs**: Once the EDR agent is deployed, "Run Scan" uses real Trivy CVE data from your actual machine instead of mock data.
- **Deterministic, no AI hallucinations**: Every finding is generated by rule-based engines — no LLM-generated false positives.
...
**Fix:** Download Trivy from [https://github.com/aquasecurity/trivy/releases](https://github.com/aquasecurity/trivy/releases) and ensure it's in the system PATH. The agent checks `shutil.which("trivy")` before scanning.
